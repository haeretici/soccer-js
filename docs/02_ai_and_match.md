...
* **Presets (revamp):** `balanced`, `tiki_taka`, `catenaccio`, `gegenpressing`, `structured_counter`, `wing_play`, `route_one` — each carries formation hold / support / press / pass aggression **and** attack line depth, region shift, role bias, own-half support blend, form pull, push scale, support width. Tuned via headless eval (`node scripts/eval_ai_presets.js`) so styles remain match-viable (goals, shots, progressive passes) with distinct identities.
* **Dynamic Strategy:** Teams shift to `gegenpressing` (losing late) or `catenaccio` (winning late) if `dynamicStrategyShifting` is enabled — **strategy knobs only** (attack shape chosen at kickoff is preserved).
